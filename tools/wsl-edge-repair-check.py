...
def run_case(name, apply, mode='', corrupt=False, drift=False):
    with tempfile.TemporaryDirectory(prefix='inspection-repair-test-') as temp:
        fixture = Path(temp)
        root, source, backup = [fixture/p for p in ('target', 'source', 'backup')]
        root.mkdir(); source.mkdir(); backup.mkdir()
        script = SCRIPT
        before = {}
        ...
        for old_hash, relative in deps:
            target = root/relative
            target.parent.mkdir(parents=True, exist_ok=True)
            content = ('shared:' + relative).encode()
            target.write_bytes(content)
            script = script.replace(old_hash, digest(content))
        if corrupt:
...
'''
        ...
        expected_success = not (corrupt or drift or mode)
        ...
        for relative, *_ in entries:
            ...
            elif not apply or corrupt or drift:
